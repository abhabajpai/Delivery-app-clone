...
const AddToCartButton = ({ product, size }: ButtonProps) => {
  ...
  return itemCount > 0 ? (
    <div
      className={`flex h-full w-full justify-around rounded-lg uppercase font-bold text-sm cursor-pointer ${size === 'lg' ? 'text-lg' : 'text-normal'} `}
      style={{ background: 'var(--zepto-pink)' }}
    >
      <button
        onClick={(e) => remove(e)}
        type="button"
